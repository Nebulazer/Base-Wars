...

{
	...
	_index = -1;
	{
		if ( getNumber( configFile >> "CfgWeapons" >> _className >> "type" ) isEqualTo _x ) exitWith {
			_index = _forEachIndex;
		};
		
	}forEach [ 1, 4, 2 ]; //These are the vanilla defines 'type' for primary, secondary, handgun for each weapon in CfgWeapons
